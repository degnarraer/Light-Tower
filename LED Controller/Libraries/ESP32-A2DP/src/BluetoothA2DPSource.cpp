...
void BluetoothA2DPSource::filter_inquiry_scan_result(esp_bt_gap_cb_param_t *param)
{
	...
	ESP_LOGD(BT_AV_TAG, "Scanned device: %s", to_str(param->disc_res.bda));
    for (int i = 0; i < param->disc_res.num_prop; i++) {
        p = param->disc_res.prop + i;
        switch (p->type) {
        case ESP_BT_GAP_DEV_PROP_COD:
            cod = *(uint32_t *)(p->val);
            ESP_LOGD(BT_AV_TAG, "--Class of Device: 0x%x", cod);
            break;
        case ESP_BT_GAP_DEV_PROP_RSSI:
            rssi = *(int8_t *)(p->val);
            ESP_LOGD(BT_AV_TAG, "--RSSI: %d", rssi);
            break;
    ...
    if ((esp_bt_gap_get_cod_major_dev(cod) == ESP_BT_COD_MAJOR_DEV_AV))
	{
		...
		if (eir)
		{
			ESP_LOGD(BT_AV_TAG, "--Compatiblity: Compatible");
			get_name_from_eir(eir, s_peer_bdname, NULL);
			ESP_LOGD(BT_AV_TAG, "--Name: %s", s_peer_bdname);
