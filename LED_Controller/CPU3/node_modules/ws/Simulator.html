...
<body>
    ...
    <iframe src="../../data/index.html" title="Content Frame" width="600" height="600"></iframe>
    ...
    <script>
        ...
        socket.onmessage = (event) => {
            if (event.data instanceof Blob) {
                blobToString(event.data)
                    .then(strMessage => processMessage(JSON.parse(strMessage)))
                    .catch(error => console.error('Simulator: Error converting blob to string:', error));
            } else if (typeof event.data === 'string') {
                processMessage(JSON.parse(event.data));
            } else {
                console.error('Simulator: Received unsupported data type:', typeof event.data);
            }
        };
        ...
        function blobToString(blob) {
            return new Promise((resolve, reject) => {
                ...
                reader.onerror = (event) => reject(new Error("Failed to read blob: " + event.target.error));
                reader.readAsText(blob);
            });
        }
    
        function processMessage(message) {
            if (message.SignalValue) {
                const { Id, Value } = message.SignalValue;
        ...
        function sendSignal(signalId) {
            const inputField = document.getElementById(signalId);
            const value = inputField.value;
    
            if (!value) {
                console.warn(`Simulator: No value provided for ${signalId}`);
                return;
            }
